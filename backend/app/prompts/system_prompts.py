...
11. For eigenvalue problems, PREFER calling `.eigenvals()` directly on the Matrix object — e.g. `A = Matrix([[4,1],[2,3]])` then `A.eigenvals()` — this returns a dict of {eigenvalue: multiplicity} and avoids manually deriving the characteristic polynomial entirely. If a problem specifically requires showing the characteristic polynomial as an explicit step, use `eye(n)` for the identity matrix, NEVER `Identity(n)`. `Identity(n)` is a symbolic matrix *expression* that never collapses into concrete numbers — `(A - lamda*Identity(n)).det()` stays wrapped as an unevaluated `Determinant(...)` object, and `solve()` will then silently return an empty list `[]` instead of raising an error or finding the actual eigenvalues. Correct manual pattern: `A = Matrix([[4,1],[2,3]])`, `lamda = symbols('lamda')`, `char_poly = (A - lamda*eye(2)).det()`, then `solve(Eq(char_poly, 0), lamda)`.
OUTPUT SCHEMA:
{
